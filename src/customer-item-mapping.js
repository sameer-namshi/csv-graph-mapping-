...
const createCustomerPurchaseRelationship = async (tx, salesDetails) => {
    ...
    const {order_date_key, order_nr, fk_customer, 
        sku, sku_config, id_sales_order_item, sale_price} = salesDetails

    const result = await tx.run(`
    MATCH (product:Product {sku_config: $skuConfig})
    MATCH (simple:ProductSimple {sku: $sku})
    ...
    RETURN product.sku_config, customer.id, order.order_nr, simple.sku
    `, 
    {
